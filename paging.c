...
// সব স্ট্যাটিক, 1MB এর নিচে থাকবে তাই অটো ম্যাপড
...
page_t *get_page(uint32_t address, int make, page_directory_t *dir) {
    ...
    if (table_idx == 0) {
        return &dir->tables[0]->pages[address % 1024];
    }
...
void paging_init() {
    ...
    memset(frames, 0, sizeof(frames)); // স্ট্যাটিক অ্যারে ক্লিয়ার

    memset(kernel_directory, 0, sizeof(page_directory_t));
    memset(&first_page_table, 0, sizeof(page_table_t));

    // CPU কে tablesPhysical অ্যারের ফিজিক্যাল অ্যাড্রেসটা দিতে হবে
    kernel_directory->physicalAddr = (uint32_t)kernel_directory->tablesPhysical;
    
    kernel_directory->tables[0] = &first_page_table;
    kernel_directory->tablesPhysical[0] = (uint32_t)&first_page_table | 0x3; // Present, RW
    ...
    // 0-4MB Identity Map - Kernel + Stack + Heap + VGA সব কভার
