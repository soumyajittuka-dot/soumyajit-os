...
#include "keyboard.h"
#include "e1000.h"
#include "ports.h" // meminfo er jonno lagbe

char input_buffer[256];
int input_pos = 0;

// String compare functions same rakhlam
...
void execute_command(char* cmd) {
    ...
    if(string_equal(cmd, "clear")) {
        ...
        *bad = 123; // Page Fault ISR14
        ...
        volatile int a = 10;
        volatile int b = 0;
        volatile int c = a / b; // ISR0 - Divide by Zero
        (void)c;
        vga_print_string("You should NOT see this!\n", VGA_COLOR_RED);
    }
    else if(string_equal(cmd, "about")) {
        vga_print_string("Soumyajit OS v2.2\n", VGA_COLOR_GREEN);
        vga_print_string("Features: IDT, Paging, GFX, e1000\n", VGA_COLOR_GREEN);
        ...
        extern volatile uint32_t timer_ticks; // timer.c e declare korte hobe
        ...
        __asm__ volatile("hlt"); // CPU halt if reboot fails
    }
    else if(string_equal(cmd, "color")) {
        vga_print_string("VGA Colors:\n", VGA_COLOR_WHITE);
        for(int i = 0; i < 16; i++) {
            vga_print_string("Color ", i);
            // Color number print korte chaile aro code lagbe
            vga_print_string("### ", i);
        }
        vga_print_string("\n", VGA_COLOR_WHITE);
    }
...
void process_keyboard_input(char c) {
    if(c == '\n') {
        ...
        vga_print_string("SOC> ", VGA_COLOR_LIGHT_BLUE); // FIX: Eikhane prompt
    }
    else if(c == '\b') {
        if(input_pos > 0) {
            input_pos--;
