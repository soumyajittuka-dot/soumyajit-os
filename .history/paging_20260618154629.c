...
page_t *get_page(uint32_t address, int make, page_directory_t *dir) {
    ...
    if (dir->tables[table_idx]) {
        ...
        // ফিক্স: রিকার্সিভ কল এ `dir` পাস করতে হবে
...
void paging_init() {
    ...
    // 0x0 থেকে 16MB আইডেন্টিটি ম্যাপ
    for (uint32_t i = 0; i < 0x1000000; i += 0x1000) {
        alloc_frame_physical(get_page(i, 1, kernel_directory), 1, 1, i);
    }

    // ফিক্স: VGA বাফার - Kernel + Writeable হতে হবে
    for (uint32_t i = 0xA0000; i < 0xC0000; i += 0x1000) {
        alloc_frame_physical(get_page(i, 1, kernel_directory), 1, 1, i); // 0,0 -> 1,1
    }
